...
class Create extends Component {
  ...
  normaliseForm = (formdata, entity) => {
    if (entities[entity].postData) {
      let postdata = [];
      entities[entity].postData.map(
        (field) => (postdata[field] = formdata[field]),
      );
      return postdata;
    }
    return formdata;
  ...
  updateEntity = () => {
    const {id, entity} = this.props.match.params;
    const formdata = this.normaliseForm(this.state.formdata, entity);
  ...
  handleEntityAction = (action) => {
    ...
    if (entity === 'geolocation') {
      if (
        !this.state.formdata.tariff_code ||
        this.state.formdata.tariff_code === ''
      ) {
        this.setState({error: true, errortext: 'Tariff Code can not be empty'});
        return;
      }
    }
