...
class EnhancedUserTable extends React.Component {
  ...
  componentDidMount() {
    ...
    if(searchPayload && searchPayload.entity && (searchPayload.entity == this.props.entity)) {
      ...
      const {entity, s, ...dataToSend} = searchPayload;
  ...
  saveTableData = () => {
    ...
    const statePayload = this.getSearchPayload(true);
  ...
  getSearchPayload = (realPageNo = false) => {
  ...
  getList = (entity, data) => {
    this.fetchList(entity, data)
      .then((response) => {
        ...
        return this.setState({page: 0, data, loading: false, total, rowsPerPage: parseInt(per_page)}, () => this.saveTableData());
      });
  };
  ...
  handleChangePage = (event, page) => {
    ...
    this.setState({page}, () => this.saveTableData());
  };
