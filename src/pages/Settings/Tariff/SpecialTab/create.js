...
class CreateForm extends Component {
    ...
    if (this.props.edit && !this.state.inited && nextProps.existingTariff) {
      ...
      if(nextProps.existingTariff.condition) {
        ...
        if(conditions.length) {
          ...
          const settingsData = conditions.map(
            condition => ({
              ...condition.source_dropdown,
              selectedValue: condition.value_condition,
              selected: condition.source_dropdown.special_tariff_condition_id
            })
          )
  ...
  getSettingCondition = () => {
    let { conditionsFromApi = [] } = this.state;
    conditionsFromApi = conditionsFromApi || [];
    const conditionData = [...conditionsFromApi];
  ...
  removeSettings = index => e => {

    const { settingsData } = this.state;

    const copiedSettingsData = [...settingsData];

    this.setState({
      settingsData: copiedSettingsData.filter((setting, existingIndex) => existingIndex !== index)
    })
                    ...
                    <Grid container>
                      ...
                      <Grid item xs={6} sm={6}>
                        { condition.selected  ?
                          ...
                          ></TextField>
                        : null}
                        {!viewOnly && condition.selected ? <Button style={{float: 'right'}} onClick={this.removeSettings(index)}>Remove</Button> : null }
                      </Grid>
                    </Grid>
                  </Grid> 
                )
