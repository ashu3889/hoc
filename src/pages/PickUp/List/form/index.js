...
const MapWithAMarker = withScriptjs(
  withGoogleMap((props) => {
    ...
    let searchBoxRef;
    return (
      <GoogleMap defaultZoom={15} defaultCenter={{lat: lat, lng: lng}} center={{lat: lat, lng: lng}} onClick={props.onClick}>
        <SearchBox
        ...
        <ToolTipMarker
          draggable={true}
          onDragEnd={props.onUpdateMarker}
...
class PickRequestForm extends Component {
  ...
  handleChange = (name) => (event) => {
    ...
    if (event.target.name === 'req_phone') {
      value = value.replace(/[^0-9]/, '');
    }
    if (event.target.name === 'amount_payment') {
    ...
    this.geoCoder.geocode({ latLng }, (results, status) => {
      if (status === google.maps.GeocoderStatus.OK && results[0]) {
        this.setState({
          ...
          city_name: results[0].address_components.find(address => address.types.includes('administrative_area_level_2')).short_name,
        });
      }
    });
    this.setState({
    ...
    if (searchBox) {
      ...
      if (places && places.length > 0 && typeof places[0].address_components !== 'undefined') {
        this.setState({
          ...
          city_name: places[0].address_components.find(address => address.types.includes('administrative_area_level_2')) ? places[0].address_components.find(address => address.types.includes('administrative_area_level_2')).short_name : '',
  ...
  handleMapClick = (name) => (event) => {
    ...
    this.setState({
      req_lat: event.latLng.lat(),
      req_lon: event.latLng.lng(),
    });
    ...
    if (this.props.edit !== true) {
      ...
      getEntity(`pickup/${id}`, null).then((response) => {
        ...
        this.setState({
          ...
          req_courier_employee_id: data.req_courier_employee_id,
  ...
  entitySubmitSuccess = () => {
    ...
    this.props.history.push(`/pickup/list`);
  };
  ...
  render() {
    ...
    const customerTypeData = ['Agent', 'Customer', 'Corporate'];
    return (
      <BlockUI tag="div" blocking={isLoading}  className={classes.root}>
          ...
          <Grid md={12} item>
            <Paper className={classes.formWrapper}>
              ...
              <Grid container>
                <Grid item xs={6} sm={6}>
                  <div>
                    <div className={classes.nameWrapper}>
                      ...
                      <IconButton tabIndex={'-1'} className={classes.searchbutton} onClick={this.handleCustomerSearchDialog}>
                        <Person />
                      </IconButton>
                    </div>
                    <br />
                ...
                <Grid item xs={6} sm={6} className={classes.rightPart}>
                  <div>
