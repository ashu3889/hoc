        ...
        />
          ...
          <Table id="printableTable">
            ...
            <TableBody
              ...
              render={(datum, index) => {
                if(datum.createdOn){
                  var createdOn = <Moment format="DD MMM/HH:mm">
                                    {datum.createdOn}
                                  </Moment>;
                } else {
                  var createdOn = "-";
                ...
                return (
                  <TableRow key={index}>
                    ...
                    <TableCell numeric>{datum.additionalInfo ? datum.additionalInfo.totalWeight : 0}</TableCell>
                    <TableCell numeric>{datum.additionalInfo ? datum.additionalInfo.totalConnote : 0}</TableCell>
                    <TableCell numeric>{datum.total_bag_count ? datum.total_bag_count : '-'}</TableCell>
                    {
                      datum.statusDetail && datum.statusDetail.value === 'FINISH' &&
                      <TableCell>
                        <Route render={({ history}) => (
                          <Button
                            style={{color: '#343b91'}}
                            onClick={() => history.push(`/transport/manifest/check/${datum.manifestID}`)}
                          >
                            CHECK
                          </Button>
                        )} />
