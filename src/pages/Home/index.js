...
const styles = (theme) => ({
  ...
  breadCrumbs: {
      ...baseStyles(theme).breadCrumbs,
  },
  transactionBreadcrumbs: {
      ...baseStyles(theme).transactionBreadcrumbs,
  },
  titleWrapper: {
...
class Index extends React.Component {
  constructor() {
    ...
    this.state = {
      ...
      methodData: 'Cash',
      packageItem: [],
      searchResultAlmt: {error: 'Data Not Found'},
      allData: [],
      /*_VCW: 0,
      _CW: 0,
      _AW: 0,*/
  ...
  finishTransaction = (event) => {
    const {destReducer} = this.props.additionalStore;
    if (destReducer.namaPenerima) {
      this.handleSubmitAllForms();
    } else {
  ...
  createConnote = (endpoint) => {
    const {otherInfoReducer, originReducer, destReducer} = this.props.additionalStore;
    const {deskripsiBrg, pcs, service, koli, remarks, insuredVal,single_connote_no,pra_connote} = otherInfoReducer;
    ...
    const wrappedObj = {
      package: {
        ...
        OtherInfoData: {deskripsiBrg, pcs, service, koli,single_connote_no,pra_connote, remarks, insuredVal, insuranceFee: insuredVal > 0 ? Math.floor((insuredVal * 0.002) * 100) / 100 : 0, insuranceAdminFee: insuredVal > 0 ? 5000 : 0},
  ...
  pushDataConnote(response) {
    const {connoteData} = this.state;
    connoteData.push(response.data.data.connote);
    this.setState({connoteData:connoteData});
  }
  printConnote = (event) => {
  ...
  doneTyping(almt, zip) {
    ...
    getEntityList(zip ? 'geolocation/searchzipcodes' : 'geolocation/search', {s: almt}, source.token).then((response) => {
      ...
      source = null;
      return this.setState({searchResultAlmt: data || {error: 'Data Not Found'}});
    }).catch((thrown) => {
      if (axios.isCancel(thrown)) {
        //console.log('Request canceled', thrown.message);
      } else {
        // handle error
        this.setState({searchResultAlmt: {error: thrown.message}});
      }
    });
  }
  handleBlurSearch = (e) => {
  ...
  componentDidMount = () => {
    const {fetchAlmtPenerima, resetForm} = this.props;
    resetForm();
    getEntityList('surcharges?n='+sessionStorage.getItem('userNodeId'))
    .then((response) => {
      this.setState({surcharges: response.data.data});
    }).catch((error) => {
      console.log(error);
    });
    getEntityList('surcharges_ow?n='+sessionStorage.getItem('userNodeId'))
    .then((response) => {
      this.setState({surcharges_ow: response.data.data});
    }).catch((error) => {
      console.log(error);
    });
  }
  onChangePaymentType = (event) => {
    this.setState({methodData: event.target.value});
  }
  render() {
    const {
      handlePackageChange,
    ...
    const {connote_code,connoteData} = this.state;
    return (
