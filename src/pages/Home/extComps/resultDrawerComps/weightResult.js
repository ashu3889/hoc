...
class WeightResult extends React.Component {
  render() {
    ...
    const data = otherInfoReducer.serviceData.filter((item) => { return item.service_code === otherInfoReducer.service; });
    const min_weight = data.length ? data[0].tarif.min_weight : 0;
    otherInfoReducer.koli.map((item) => {
      const panjang = parseInt(item.panjang, 10);
      const pk = item.pkType;
      ...
      let actualWeight = parseFloat(isNaN(item.berat) || item.berat === '' ? 0 : item.berat);
      let actualCw = 0;
      ...
      chargeableWeight = cw > 300 ? Math.ceil(chargeableWeight) : Math.floor(chargeableWeight);
      chargeableWeight = Math.max(chargeableWeight, min_weight);
