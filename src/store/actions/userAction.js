  ...
  mapDispatchToProps(dispatch) {
    return {
      ...
      fetchAlmtPenerima: (item) => {
        ...
        dispatch({type: 'USER_INTRECTION_DIRTY', payload: true});
        dispatch({type: 'SET_ALMT_TO_FIELD', payload: item});
        dispatch({type: 'THROW_KODEPOSTO_TO_DESTINATIONDATA', kodePos: item.zip_code, kodeTo: item.tariff_code});
        if (item.tariff_code && originCode) {
          dispatch({type: 'SERVICE_CODE_REQUESTED'});
          getEntityList('service', {
            origin: originCode,
            destination: item.tariff_code,
          }).then((response) => {
            const {data} = response.data;
            dispatch({type: 'SET_FOCUS_ON_INPUT', payload: item.tariff_code !== null ? 'deskripsiInput' : ''});
            dispatch({type: 'SET_OTHERINFO_INPUT', key: 'serviceData', payload: data});
          });
        }
      },
