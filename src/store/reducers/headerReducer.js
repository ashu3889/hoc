...
export default (state = initialState, action) => {
  ...
  switch (action.type) {
    case 'GET_USER_DETAIL_SUCCEED':
       ...
       */
      if (sessionStorage.getItem('userNodeId')) {
        /**
         * reload last selected node, but check if user who is signed in have access to those nodes
         */
        const sessionNode = action.user.nodes.filter((item) => item.nodeID === parseInt(sessionStorage.getItem('userNodeId')));
        if (sessionNode.length) {
          return {
            ...state,
            nodes: action.user.nodes,
            currentNode: sessionNode[0].nodeID,
            activeNode: sessionNode[0],
          };
        }
      }
      console.log('hello', sessionStorage.getItem('userNodeId'))
