...
const otherinfoData = (state = initOtherinfoData, action) => {
  switch (action.type) {
    ...
    case 'UPDATE_KOLI_SURCHARGE':
      ...
      var split_arr = action.payload.value.split("|");
      //console.log('UPDATE_KOLI_SURCHARGE', action.payload)
      return update(state, {
        koli: {
          [action.payload.koli.id]: {
            [action.payload.name]: {$set: action.payload.name === 'packingKayu' ? action.payload.checked : split_arr[0]},
            surcharge_id: {$set: action.payload.name === 'packingKayu' ? '' : split_arr[1]},
          },
        },
      });
    case 'SET_OTHERINFO_INPUT':
      ...
      if (action.key === 'adtSurcharge') {
        ...
        if (
          ...
          let surcharge_id = state.koli[action.payload.index].surcharge_id?state.koli[action.payload.index].surcharge_id:'';
          surcharge = overWeight ? 'Overweight' : (surcharge === 'Overweight' ? '' : surcharge);
          return update(state, {
            koli: {
              [action.payload.index]: {
                [action.key]: {$set: action.payload.val},
                overWeight: {$set: overWeight},
                surcharge: {$set: surcharge},
                surcharge_id: {$set: surcharge_id},
              },
            },
          });
